...
def _not_run(kind: str) -> BackendVerdict:
    return BackendVerdict(status="not_run", message=f"{kind} skipped (peer already valid)", verified_at="")
...
def _task_verdict(task: asyncio.Task, kind: str) -> BackendVerdict:
    ...
    if exc is not None:
        if isinstance(exc, BbopsUnhealthy):
            return BackendVerdict(status="not_run", message="bbops unhealthy", verified_at="")
        return BackendVerdict(status="error", message=f"{kind} timeout", verified_at="")
    res = task.result()
    return res if isinstance(res, BackendVerdict) else BackendVerdict(
        status="error", message=f"{kind} bad result", verified_at="")


async def run_backends(racknerd, bbops, email: str, mx_provider: str | None,
