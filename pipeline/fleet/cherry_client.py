...
class CherryClient:
    ...
    async def get_user(self) -> dict:
        return cast(dict, await self._request("GET", "/user"))

    async def list_teams(self) -> list[dict]:
        return cast(list, await self._request("GET", "/teams"))
    ...
    async def list_projects(self, team_id: int) -> list[dict]:
        return cast(list, await self._request("GET", f"/teams/{team_id}/projects"))

    async def list_plans(self, team_id: int) -> list[dict]:
        return cast(list, await self._request("GET", f"/teams/{team_id}/plans"))

    # --- SSH keys -----------------------------------------------------------
    async def list_ssh_keys(self) -> list[dict]:
        return cast(list, await self._request("GET", "/ssh-keys"))

    async def create_ssh_key(self, label: str, public_key: str) -> dict:
        return cast(dict, await self._request("POST", "/ssh-keys", body={"label": label, "key": public_key}))
    ...
    async def list_servers(self, project_id: int) -> list[dict]:
        return cast(list, await self._request("GET", f"/projects/{project_id}/servers"))
    ...
    async def create_server(
        ...
        *,
        ...
        if tags:
            body["tags"] = tags
        return cast(dict, await self._request("POST", f"/projects/{project_id}/servers", body=body))

    async def get_server(self, server_id: int) -> dict:
        return cast(dict, await self._request("GET", f"/servers/{server_id}"))
